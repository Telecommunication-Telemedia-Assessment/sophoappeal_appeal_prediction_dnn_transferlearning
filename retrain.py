...
def make_generator(df, image_size, batch_size, shuffle=True):
...
def get_train_val_generators(image_size, batch_size, basedir):
    ...
    df = pd.read_csv(os.path.join(basedir, "../images/all_features.csv")) #.head(1000)


    df["image"] = df["image"].apply(lambda x: os.path.join(basedir, "../images/" + x))
    ...
    image_gen_validation = make_generator(dval, image_size, batch_size, shuffle=False)
...
def main(args):
    ...
    ax.get_figure().savefig("hist_retrain.pdf")
    dh.to_csv("hist_retrain.csv", index=False)
    ...
    truth = []
    i = 0
    filepaths = list(image_gen_validation.filepaths)
    ...
    print(dr.corr())
    dr.to_csv("predictions.csv", index=False)
    ax = dr.plot(x="truth", y="prediction", kind="scatter")
    ax.get_figure().savefig("scatter_retrain.pdf")
